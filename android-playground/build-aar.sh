...
AAR_OUTPUT_DIR="$PROJECT_ROOT/android-lib/build/outputs/aar"
...
case $BUILD_ENV in
    debug)
        ...
        ./gradlew :android-lib:assembleDebug $GRADLE_ARGS
        AAR_FILE="$AAR_OUTPUT_DIR/android-lib-debug.aar"
        ;;
    release)
        print_info "构建 release 版本..."
        ./gradlew :android-lib:assembleRelease $GRADLE_ARGS
        AAR_FILE="$AAR_OUTPUT_DIR/android-lib-release.aar"
        ;;
    release-minified)
        print_info "构建混淆的 release 版本..."
        # 临时修改 build.gradle.kts 启用混淆
        sed -i.bak 's/isMinifyEnabled = false/isMinifyEnabled = true/' android-lib/build.gradle.kts
        ./gradlew :android-lib:assembleRelease $GRADLE_ARGS
        # 恢复原始配置
        mv android-lib/build.gradle.kts.bak android-lib/build.gradle.kts
        AAR_FILE="$AAR_OUTPUT_DIR/android-lib-release.aar"
        ;;
esac
...
if [ -f "$AAR_FILE" ]; then
    ...
    # 复制到 UniApp 项目（如果存在）
    UNIAPP_AAR_DIR="$PROJECT_ROOT/../uniapp-x-playground/uni_modules/say-hi/utssdk/app-android/libs"
