...
set "AAR_OUTPUT_DIR=%PROJECT_ROOT%sse-lib\build\outputs\aar"
...
if "%BUILD_ENV%"=="debug" (
    ...
    gradlew.bat :sse-lib:assembleDebug %GRADLE_ARGS%
    set "AAR_FILE=%AAR_OUTPUT_DIR%\sse-lib-debug.aar"
) else if "%BUILD_ENV%"=="release" (
    echo [INFO] 构建 release 版本...
    gradlew.bat :sse-lib:assembleRelease %GRADLE_ARGS%
    set "AAR_FILE=%AAR_OUTPUT_DIR%\sse-lib-release.aar"
) else if "%BUILD_ENV%"=="release-minified" (
    echo [INFO] 构建混淆的 release 版本...
    REM 临时修改 build.gradle.kts 启用混淆
    powershell -Command "(Get-Content sse-lib\build.gradle.kts) -replace 'isMinifyEnabled = false', 'isMinifyEnabled = true' | Set-Content sse-lib\build.gradle.kts"
    gradlew.bat :sse-lib:assembleRelease %GRADLE_ARGS%
    REM 恢复原始配置
    powershell -Command "(Get-Content sse-lib\build.gradle.kts) -replace 'isMinifyEnabled = true', 'isMinifyEnabled = false' | Set-Content sse-lib\build.gradle.kts"
    set "AAR_FILE=%AAR_OUTPUT_DIR%\sse-lib-release.aar"
)
...
if exist "%AAR_FILE%" (
    ...
    set "UNIAPP_AAR_DIR=%PROJECT_ROOT%..\sse-uniapp-demo\uni_modules\sse-plugin\utssdk\app-android\libs"
