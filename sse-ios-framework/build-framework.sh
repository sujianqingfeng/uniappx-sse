...
PROJECT_NAME="SSEFramework"
FRAMEWORK_NAME="SSEFramework"
UTS_FRAMEWORK_DIR="../sse-uniapp-demo/uni_modules/sse-plugin/utssdk/app-ios/Frameworks"
IOS_PLAYGROUND_DIR="../sse-ios-demo/SSEDemo"
...
if [ -d "$UTS_FRAMEWORK_DIR" ]; then
    ...
    # 复制新的 framework
    cp -R "$FINAL_FRAMEWORK" "$UTS_FRAMEWORK_DIR/$FRAMEWORK_NAME.framework"
...
if [ -d "$IOS_PLAYGROUND_DIR" ]; then
    ...
    # 复制新的 framework 并重命名为正确的名称
    cp -R "$FINAL_FRAMEWORK" "$IOS_PLAYGROUND_DIR/$FRAMEWORK_NAME.framework"
